...
for i in file_read_list:
    ...
    for j in i:
        ...
        index += 1
    weak_x_list.append(new_list)

y_distance_new = y_distance[:1001]
for i in range(len(weak_x_list)):
    sexy_boi.plot(weak_x_list[i], y_distance_new)
...
initial_velocity_list = list(range(30, 121, 30))
air_resistance_file = file_read_list[3]

new_x_initial_velocity = []
new_y_initial_velocity = []

for i in initial_velocity_list:
    new_x_initial_velocity.append(i * yourmom.cos(angle_of_traj))
    new_y_initial_velocity.append(i * yourmom.sin(angle_of_traj))
